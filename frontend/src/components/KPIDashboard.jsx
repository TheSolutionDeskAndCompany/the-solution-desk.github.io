      ...
      <div className="chart-container">
        ...
        <ResponsiveContainer width="100%" height={400}>
          <LineChart data={monthlyData}>
            ...
            <Line type="monotone" dataKey="revenue" stroke="#66fcf1" strokeWidth={2} dot={{ fill: '#66fcf1', r: 4 }} />
            <Line type="monotone" dataKey="expenses" stroke="#ff2e63" strokeWidth={2} dot={{ fill: '#ff2e63', r: 4 }} />
            <Line type="monotone" dataKey="profit" stroke="#45a29e" strokeWidth={2} dot={{ fill: '#45a29e', r: 4 }} />
          </LineChart>
        </ResponsiveContainer>
      </div>
    );
              ...
              <BarChart data={teamPerformanceData}>
                ...
                <Bar dataKey="performance" fill="#45a29e">
                  {teamPerformanceData.map((entry, index) => (
                    <Cell 
                      key={`cell-${index}`} 
                      fill={entry.performance >= 80 ? '#66fcf1' : entry.performance >= 70 ? '#45a29e' : '#ff2e63'} 
                    />
                  ))}
                </Bar>
              </BarChart>
  ...
  return (
    <div className="kpi-container cyberpunk-theme">
      ...
      <p className="kpi-description">
        Real-time metrics and analytics tracking our company's performance across different departments.
      </p>
      ...
      <div className="dashboard-content">
        {activeView === 'financial' ? renderFinancialMetrics() : renderProjectMetrics()}
      </div>
    </div>
  );
}
